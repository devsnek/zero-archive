...
({ binding, load, process }) => {
  ...
  function CFI(fn, returnType, argTypes) {
    ...
    let lazyEncoder;
    if (LazyTextEncoder === undefined) {
      LazyTextEncoder = load('whatwg/encoding').TextEncoder;
    }
      ...
      args.forEach((arg, i) => {
        ...
        if (type === ffi.types.int) {
          ...
          if (typeof arg === 'string') {
            if (lazyEncoder === undefined) {
              lazyEncoder = new LazyTextEncoder();
            }
            const b = lazyEncoder.encode(`${arg}\0`);
