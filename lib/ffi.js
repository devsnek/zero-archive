...
({ binding, load, process, PrivateSymbol: PS }) => {
  ...
  const dlopen = CFI(ffi.dlopen, 'pointer', ['cstring', 'int']);
  const dlsym = CFI(ffi.dlsym, 'pointer', ['pointer', 'cstring']);
  // const dlerror = CFI(ffi.dlerror, 'void', []);
  // const dlclose = CFI(ffi.dlclose, 'void', ['pointer']);

  let lazyURL;

  const loadDynamicLibrary = (file, functions) => {
    if (file !== null) {
    ...
    const handle = dlopen(file, ffi.RTLD_LAZY);
