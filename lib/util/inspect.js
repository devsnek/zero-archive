'use strict';

({ namespace, binding }) => {
  const {
    getProxyDetails,
    getPromiseDetails,
    kPending,
    kRejected,
  } = binding('util');
  const ScriptWrap = binding('script_wrap');
  const {
    isRegExp,
    isDate,
    isModuleNamespaceObject,
  } = binding('types');

  const uncurryThis = (fn) => (t, ...a) => Reflect.apply(fn, t, [a]);
  const dateToISOString = uncurryThis(Date.prototype.toISOString);
  const regExpToString = uncurryThis(RegExp.prototype.toString);
  const errorToString = uncurryThis(Error.prototype.toString);

  const {
    previewMapIterator,
    previewSetIterator,
    previewWeakMap,
    previewWeakSet,
    isMap,
    // isSet,
    isWeakMap,
    isWeakSet,
    isMapIterator,
    isSetIterator,
  } = ScriptWrap.run('NA', `(
{
  previewMapIterator: (it) => %MapIteratorClone(it),
  previewSetIterator: (it) => %SetIteratorClone(it),
  previewWeakMap: (weakMap, max = 0) => %GetWeakMapEntries(weakMap, max),
  previewWeakSet: (weakSet, max = 0) => %GetWeakSetValues(weakSet, max),
  isMap: (v) => %IsJSMap(v),
  // isSet: (v) => %IsJSSet(v),
  isWeakMap: (v) => %IsJSWeakMap(v),
  isWeakSet: (v) => %IsJSWeakSet(v),
  isMapIterator: (v) => %IsJSMapIterator(v),
  isSetIterator: (v) => %IsJSSetIterator(v),
})`);

  // compile natives syntax
  previewMapIterator(new Map().values());
  previewSetIterator(new Set().values());
  previewWeakMap(new WeakMap());
  previewWeakSet(new WeakSet());
  isMap(0);
  // isSet(0);
  isWeakMap(0);
  isWeakSet(0);
  isMapIterator(0);
  isSetIterator(0);

  const ansi = Object.assign(Object.create(null), {
    bold: [1, 22],
    italic: [3, 23],
    underline: [4, 24],
    inverse: [7, 27],
    white: [37, 39],
    grey: [90, 39],
    black: [30, 39],
    blue: [34, 39],
    cyan: [36, 39],
    green: [32, 39],
    magenta: [35, 39],
    red: [31, 39],
    yellow: [33, 39],
  });

  const styles = Object.assign(Object.create(null), {
    special: 'cyan',
    bigint: 'blue',
    number: 'blue',
    boolean: 'yellow',
    undefined: 'grey',
    null: 'bold',
    string: 'green',
    symbol: 'green',
    date: 'magenta',
    regexp: 'red',
  });

  const stylize = (str, styleName) => {
    const style = styles[styleName];
    if (style !== undefined) {
      const [start, end] = ansi[style];
      return `\u001b[${start}m${str}\u001b[${end}m`;
    }
    return str;
  };

  const meta = [
    '\\u0000', '\\u0001', '\\u0002', '\\u0003', '\\u0004',
    '\\u0005', '\\u0006', '\\u0007', '\\b', '\\t',
    '\\n', '\\u000b', '\\f', '\\r', '\\u000e',
    '\\u000f', '\\u0010', '\\u0011', '\\u0012', '\\u0013',
    '\\u0014', '\\u0015', '\\u0016', '\\u0017', '\\u0018',
    '\\u0019', '\\u001a', '\\u001b', '\\u001c', '\\u001d',
    '\\u001e', '\\u001f', '', '', '',
    '', '', '', '', "\\'", '', '', '', '', '',
    '', '', '', '', '', '', '', '', '', '',
    '', '', '', '', '', '', '', '', '', '',
    '', '', '', '', '', '', '', '', '', '',
    '', '', '', '', '', '', '', '', '', '',
    '', '', '', '', '', '', '', '\\\\',
  ];


  /* eslint-disable no-control-regex */
  const strEscapeSequencesRegExp = /[\x00-\x1f\x27\x5c]/;
  const strEscapeSequencesReplacer = /[\x00-\x1f\x27\x5c]/g;
  /* eslint-enable no-control-regex */

  const keyStrRegExp = /^[a-zA-Z_][a-zA-Z_0-9]*$/;

  const strEscape = (str) => {
    if (str.length < 5000 && !strEscapeSequencesRegExp.test(str)) {
      return `'${str}'`;
    }
    return `'${str.replace(strEscapeSequencesReplacer, (s) => meta[s.charCodeAt(0)])}'`;
  };

  const getConstructorName = (obj) => {
    while (obj) {
      const descriptor = Object.getOwnPropertyDescriptor(obj, 'constructor');
      if (descriptor !== undefined &&
          typeof descriptor.value === 'function' &&
          descriptor.value.name !== '') {
        return descriptor.value.name;
      }

      obj = Object.getPrototypeOf(obj);
    }

    return '';
  };

  const getPrefix = (ctor, tag) => {
    if (ctor !== '') {
      if (tag !== '' && ctor !== tag) {
        return `${ctor} [${tag}] `;
      }
      return `${ctor} `;
    }

    if (tag !== '') {
      return `[${tag}] `;
    }

    return '';
  };

  const inspect = (value, options = {}) => {
    if (!options.ctx) {
      options.ctx = {
        seen: [],
        indent: 1,
      };
    }
    if (value === null) {
      return stylize('null', 'null');
    }
    if (typeof value === 'number') {
      if (Object.is(value, -0)) {
        return stylize('-0', 'number');
      }
      return stylize(`${value}`, 'number');
    }
    if (typeof value === 'bigint') { // eslint-disable-line valid-typeof
      return stylize(`${value}n`, 'bigint');
    }
    if (typeof value === 'symbol') {
      return stylize(value.toString(), 'symbol');
    }
    if (typeof value === 'string') {
      return stylize(strEscape(value), 'string');
    }
    if (typeof value !== 'object' && typeof value !== 'function') {
      return stylize(`${value}`, typeof value);
    }

    if (isRegExp(value)) {
      return stylize(regExpToString(value), 'regexp');
    }

    if (isDate(value)) {
      if (Number.isNaN(value.getTime())) {
        return stylize(value.toString(), 'date');
      }
      return stylize(dateToISOString(value), 'date');
    }

    const proxy = getProxyDetails(value);
    if (proxy !== undefined) {
      return `Proxy [ ${inspect(proxy[0], options)}, ${inspect(proxy[1], options)} ]`;
    }

    const promise = getPromiseDetails(value);
    if (promise !== undefined) {
      const [state, result] = promise;
      if (state === kPending) {
        return 'Promise { <pending> }';
      } else if (state === kRejected) {
        return `Promise { <rejected> ${inspect(result, options)} }`;
      }
      return `Promise { ${inspect(result, options)} }`;
    }

    if (value instanceof Error) {
      return value.stack || `[${errorToString.call(value)}]`;
    }

    const { ctx } = options;

    if (ctx.seen.includes(value)) {
      return stylize('[Circular]', 'special');
    }

    const ctor = getConstructorName(value);
    let tag = value[Symbol.toStringTag];
    if (typeof tag !== 'string') {
      tag = '';
    }
    const prefix = getPrefix(ctor, tag);

    ctx.seen.push(value);

    if (typeof value === 'function') {
      const name = `${tag || 'Function'}${value.name ? `: ${value.name}` : ''}`;
      ctx.seen.pop();
      return stylize(`[${name}]`, 'special');
    }

    if (value[Symbol.iterator]) {
      const items = do {
        if (isWeakSet(value)) {
          previewWeakSet(value);
        } else if (isWeakMap(value)) {
          previewWeakMap(value);
        } else if (isSetIterator(value)) {
          previewSetIterator(value);
        } else if (isMapIterator(value)) {
          previewMapIterator(value);
        } else {
          [...value];
        }
      };
      let ms = items.length === 0 ? '' : '  '.repeat(ctx.indent);
      const i = do {
        if (isMap(value) || isWeakMap(value)) {
          const t = items.map(([k, v]) => `${inspect(k, options)} => ${inspect(v, options)}`);
          if (t.length > 3) {
            ms = '';
            const indent = '  '.repeat(ctx.indent);
            `\n${indent}${t.join(`,\n${indent}`)}\n`;
          } else {
            t.join(', ');
          }
        } else {
          items.map((v) => inspect(v, options)).join(', ');
        }
      };
      ctx.seen.pop();
      const bracket = Array.isArray(value) ? ['[', ']'] : ['{', '}'];
      return `${prefix === 'Array ' ? '' : prefix}${bracket[0]}${ms}${i}${ms}${bracket[1]}`;
    }

    const keys = Object.getOwnPropertyNames(value);
    const len = keys.length;
    const out = [];
    const isNs = isModuleNamespaceObject(value);
    for (let i = 0; i < len; i += 1) {
      const key = keys[i];
      let name;
      let str;
      let desc;
      if (isNs) {
        try {
          desc = Object.getOwnPropertyDescriptor(key, value);
        } catch (err) {
          if (err instanceof ReferenceError) {
            desc = { value: '<uninitialized>', enumerable: true };
          }
        }
      } else {
        desc = Object.getOwnPropertyDescriptor(value, key) ||
          { value: value[key], enumerable: true };
      }
      if (desc.enumerable === false) {
        continue;
      }
      if (desc.value !== undefined) {
        ctx.indent += 1;
        str = inspect(desc.value, options);
        ctx.indent -= 1;
      } else if (desc.get !== undefined) {
        if (desc.set !== undefined) {
          str = stylize('[Getter/Setter]', 'special');
        } else {
          str = stylize('[Getter]', 'special');
        }
      } else if (desc.set !== undefined) {
        str = stylize('[Setter]', 'special');
      } else {
        str = stylize('undefined', 'undefined');
      }
      if (typeof key === 'symbol') {
        name = `[${stylize(key.toString(), 'symbol')}]`;
      } else if (desc.enumerable === false) {
        name = `[${key}]`;
      } else if (keyStrRegExp.test(key)) {
        name = stylize(key, 'name');
      } else {
        name = stylize(strEscape(key), 'string');
      }

      out.push(`${name}: ${str}`);
    }

    ctx.seen.pop();

    if (out.length === 0) {
      return '{}';
    }

    const long = out.length > 4;
    const sep = long ? `\n${'  '.repeat(ctx.indent)}` : ' ';
    return `{${sep}${out.join(`,${sep}`)}${long ? `\n${'  '.repeat(ctx.indent - 1)}` : ' '}}`;
  };

  namespace.inspect = inspect;
  namespace.format = (options, ...args) => {
    if (args.length === 1 && typeof args[0] === 'string') {
      return inspect(args[0], options);
    }
    return args.map((a) => inspect(a, options)).join(' ');
  };
};
