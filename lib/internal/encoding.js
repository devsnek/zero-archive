// adapted from Node.js - https://github.com/nodejs/node/blob/master/lib/internal/encoding.js
...
({ load, binding }) => {
  const errors = load('internal/errors');
  const { encodeUtf8String } = binding('encoding');
  const { isArrayBuffer } = binding('util');
  ...
  class TextEncoder {
    ...
    get encoding() {
      if (this == null || this[kEncoder] !== true) // eslint-disable-line eqeqeq
  ...
  function makeTextDecoderICU() {
    ...
    class TextDecoder {
      ...
      decode(input = empty, options = {}) {
        if (this == null || this[kDecoder] !== true) // eslint-disable-line eqeqeq
          throw new errors.TypeError('ERR_INVALID_THIS', 'TextDecoder');

        if (!isArrayBuffer(input))
          throw new errors.TypeError('ERR_INVALID_ARG_TYPE', 'input', 'ArrayBuffer');
        if (typeof options !== 'object')
        ...
        if (typeof ret === 'number') {
          const err = new errors.TypeError('ERR_ENCODING_INVALID_ENCODED_DATA', this.encoding);
          err.errno = ret;
          throw err;
        }
  ...
  const { hasConverter, TextDecoder } = process.binding('config').hasIntl ?
    makeTextDecoderICU() : makeTextDecoderJS();
  ...
  Object.defineProperties(TextDecoder.prototype, Object.getOwnPropertyDescriptors({
    get encoding() {
      if (this == null || this[kDecoder] !== true) // eslint-disable-line eqeqeq
        throw new errors.TypeError('ERR_INVALID_THIS', 'TextDecoder');
