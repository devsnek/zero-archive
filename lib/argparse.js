...
function snekparse(args) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (/^--.+=/.test(arg)) {
      ...
      argv[match[1]] = match[2]; // eslint-disable-line prefer-destructuring
    } else if (/^--no-.+/.test(arg)) {
      ...
      for (const j of letters) {
        ...
        if (/[A-Za-z]/.test(letters[j]) && /=/.test(next)) {
          argv[letters[j]] = next.split('=')[1]; // eslint-disable-line prefer-destructuring
          broken = true;
          break;
        }
