...
({ load, process, namespace }) => {
  const { setTimeout, clearTimeout, setInterval, clearInterval } = load('whatwg/timer');
  ...
  attach('setTimeout', setTimeout);
  attach('clearTimeout', clearTimeout);
  attach('setInterval', setInterval);
  attach('clearInterval', clearInterval);
