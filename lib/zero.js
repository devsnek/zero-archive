...
`;
  ...
  utilBinding.addPromiseHook = (fn) => {
    promiseHooks.add(fn);
    if (promiseHooks.size === 1) {
      addPromiseHook((type, promise, value) => {
        for (const hook of promiseHooks) {
          try {
            hook(promiseHookTypes[type], promise, value);
          } catch {} // eslint-disable-line no-empty
        }
      });
    }
  };
  utilBinding.removePromiseHook = (fn) => {
    promiseHooks.remove(fn);
    if (promiseHooks.size === 0) {
      removePromiseHook();
    }
  };
