...
({ namespace, load }) => {
  ...
  class Event {
    constructor(type, eventInitDict = { bubbles: false, cancelable: false }) {
      this[kType] = type;
      this[kBubbles] = eventInitDict.bubbles;
  ...
  class EventTarget {
    ...
    addEventListener(type, callback, options) {
      ...
      for (let i = 0; i < this[kEventListeners][type].length; i += 1) {
        const listener = this[kEventListeners][type][i];
        if (listener.options.capture === options.capture && listener.callback === callback) {
          return;
        }
      }

      this[kEventListeners][type].push({
        callback,
        options,
      });
    }
    ...
    removeEventListener(type, callback, options) {
      ...
      for (let i = 0; i < this[kEventListeners][type].length; i += 1) {
        ...
        if (listener.callback === callback && listener.options.capture === options.capture) {
          this[kEventListeners][type].splice(i, 1);
          break;
        }
      }
    }
