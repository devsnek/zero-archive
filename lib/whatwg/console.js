...
({ load, namespace, PrivateSymbol, binding }) => {
  const { format } = load('util');
  const performance = load('w3/performance');
  const { table: cliTable } = load('cli_table');
  ...
  class Console {
    ...
    clear() {
      if (this[kStdout].isTTY) {
        const { cursorTo, clearScreenDown } = load('tty');
        cursorTo(this[kStdout], 0, 0);
        clearScreenDown(this[kStdout]);
      }
    }
    ...
    table(tabularData, properties) {
      ...
      const getIndexArray = (length) => Array.from({ length }, (_, i) => format({}, i));
    ...
    trace() {
      this[kPrint]('trace', [new Error().stack]);
    }
